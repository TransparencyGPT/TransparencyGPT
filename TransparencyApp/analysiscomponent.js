...
function AnalysisComponent() {
  ...
  const fetchAnalysis = async () => {
    ...
    const article = source.text; // This could be dynamic based on your app
    try {
  ...
  return (
    <View style={styles.container}>
      <Button title="Analyze Text" onPress={fetchAnalysis} />
      {analysisResult && (
        <View>
          <Text style={styles.heading}>Analysis Result:</Text>
          <Text>Topics: {analysisResult.topics}</Text>
          <Text>Subjectivity Score: {analysisResult.subjectivity_score}</Text>
          <Text>Analysis: {analysisResult.analysis}</Text>
        </View>
      )}
    </View>
  );
