...
completion4 = anthropic.completions.create(
    model="claude-2",
    max_tokens_to_sample=300,
    prompt=f"{HUMAN_PROMPT} Here is an article '{article}'. According to that article, did Sinner ski? Output 'not enough info' if you aren't sure! Summarize answer in just one line if you can. Just say the answer, no context{AI_PROMPT}",
)

completion5 = anthropic.completions.create(
    model="claude-2",
    max_tokens_to_sample=300,
    prompt=f"{HUMAN_PROMPT} Previous response is {completion4.completion}. Output a number, how old Sinner was when he won {AI_PROMPT}",
)
